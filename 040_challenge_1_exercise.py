...
def report_long_words(words):
  ...
  for word in words:
    if len(word) > 10:
      longer_than_ten.append(word)

  print(longer_than_ten)
  no_hyphen = []

  for word in longer_than_ten:
    if "-" not in word:
  ...
  final_words = ", ".join(shortened_words)

  return f"These words are quite long: {final_words}"
